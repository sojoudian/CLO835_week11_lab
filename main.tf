...
locals {
  ...
  k8s_token = "week11.0123456789abcdef"

  # Stage every file from manifests/ into /home/ubuntu/week11/ on the master,
  ...
  stage_manifests = join("\n", concat(
    ["", "# === Stage the Week 11 lab manifests for the ubuntu user ===", "mkdir -p /home/ubuntu/week11"],
    [for name, content in local.lab_manifests :
      "cat >/home/ubuntu/week11/${name} <<'EOF_${replace(name, ".", "_")}'\n${content}\nEOF_${replace(name, ".", "_")}"
    ],
    ["chown -R ubuntu:ubuntu /home/ubuntu/week11", ""]
  ))
}
...
#     StorageClass, and stages the lab manifests in /home/ubuntu/week11/.
########################################################
resource "aws_instance" "master" {
  ...
  # Staging runs BEFORE the kubectl-heavy init so ~/week11 exists even if a
  # later step fails.
...
resource "aws_instance" "worker" {
  ...
  tags = {
    Name = "week11-k8s-worker-${count.index + 1}"
  }
}
...
output "next_step" {
  value = "ssh -i <your-key.pem> ubuntu@${aws_instance.master.public_ip}   # then: kubectl get nodes && ls ~/week11"
}
